...
public class ChatterbookController {
    ...
    public User getSingleUsers(@RequestParam String user) {
    ...
    @RequestMapping(value = "/chatterposts", method = RequestMethod.GET)
    @ResponseStatus(value = HttpStatus.OK)
    public List<ChatterPost> getChatterPosts(@RequestParam String user) {
        List<String> listOfChatterPosts = new ArrayList<>();

        for (User username : userList) {
            if (username.getName().equals(user)) {
                return username.getChatterPosts();
            }
        }
        return null;
    }
